...
>>>>>>> Stashed changes
            	...
            	<div class="col-4 card px-1 mx-1" style="width: 21rem;">
					...
					<div class="card-body">
						...
						<ul class="list-group list-group-flush">
							<li style= "display:none" class="list-group-item" id="fIdCancha_${item.getIdCancha()}" value="${item.getIdCancha()}"> ${item.getIdCancha()}</li>
							<li style= "display:none" class="list-group-item" id="fTipoCancha_${item.getIdCancha()}" data-valor="${ item.getTipoCancha() }">${ item.getTipoCancha() }</li>
							<li class="list-group-item" id="fCantidadEquipo_${item.getIdCancha()}" data-valor="${ item.getCantidadPorEquipo() }">Cantidad Jugadores: ${ item.getCantidadPorEquipo() }</li>
							<li class="list-group-item" id="fPrecio_${item.getIdCancha()}" data-valor="${ item.getPrecio() }">Precio: ${ item.getPrecio() }</li>
							<li style="display: none" class="list-group-item" id="fType_${item.getIdCancha()}" data-valor="${ item.getType() }">${ item.getType() }</li>
						</ul>
    ...
    <script src="/Reserva_Canchas/js/canchas-futbol.js"></script>
</body>
